      ...
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-[2rem] border border-cyan-400/20 bg-slate-950/75 p-8 shadow-[0_24px_80px_rgba(8,145,178,0.18)] backdrop-blur">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.28em] text-slate-400"><EmojiAvatar emoji="📧" />{text.email}</p><p className="mt-2 break-all text-sm text-slate-200">{user.email || text.none}</p></div>
            <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.28em] text-slate-400"><EmojiAvatar emoji={active ? '✅' : '⏳'} tone={active ? 'emerald' : 'amber'} />{text.status}</p><p className={'mt-2 text-sm font-bold ' + (active ? 'text-emerald-300' : 'text-amber-300')}>{active ? 'active' : membership?.membership_status || 'pending'}</p></div>
          </div>
          <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-5"><h2 className="flex items-center gap-2 text-lg font-bold text-white"><EmojiAvatar emoji="🧭" />{text.overview}</h2><ul className="mt-4 space-y-2 text-sm leading-7 text-slate-300">{text.bullets.map((item) => <li key={item}>• {item}</li>)}</ul></div>
