...
const homeCopy = {
  'zh-Hant': {
    ...
    membershipCta: '\u6e96\u6cd5\u4eba\u7248(\u9069\u7d93\u7d00\u5546/\u81ea\u71df\u5546, EA\u4ea4\u6613\u5718\u968a, \u5bb6\u65cf\u8fa6\u516c\u5ba4\u3001\u79c1\u52df\u57fa\u91d1)',
  ...
  'zh-Hans': {
    ...
    membershipCta: '\u51c6\u6cd5\u4eba\u7248(\u9002\u7ecf\u7eaa\u5546/\u81ea\u8425\u5546, EA\u4ea4\u6613\u56e2\u961f, \u5bb6\u65cf\u529e\u516c\u5ba4\u3001\u79c1\u52df\u57fa\u91d1)',
  ...
  en: {
    ...
    membershipCta: 'Quasi-Institutional Edition (For Brokers, Prop Firms, EA Trading Teams, Family Offices & Private Funds)',
...
Object.assign(homeSecondaryCopy, {
  ja: { ...homeSecondaryCopy.ja, ui: { pricingTitle: 'サブスクリプションプラン概要', pricingBody: '上のルートを理解した後、ここが行動への自然な入口になります。', coreLabel: 'コアの利点', legalTitle: 'リスクと技術的な位置付け', legalIntro: '続行前に次の声明をお読みください。', legalBody: 'AI-Quant Lab は MQL5 ソースコード開発と AI モジュール型プロンプトエンジニアリング教育に注力しています。内容は研究と技術交流のみを目的とし、コピー取引、運用代行、利益保証、顧客資金の取扱いは行いません。取引判断とリスクは利用者自身の責任です。', faqTitle: '明確な回答が次の一歩を簡単にする', faqBody: 'サブスクリプション、MQL5 学習、戦略評価でよくある質問です。まず境界を理解し、自信を持って判断してください。', accordionTitle: 'リスクとサブスクリプションの案内', accordionBody: '使用要件、リスク境界、サブスクリプションフローを説明する追加回答です。', accordionCount: '追加の回答 3 件', faqBadge: 'FAQ / よくある質問', footerPlans: 'サブスクリプションプラン' } },
  de: { ...homeSecondaryCopy.de, ui: { pricingTitle: 'Überblick zu Abonnementplänen', pricingBody: 'Nachdem die Route klar ist, ist dies der natürliche Einstieg zur Handlung.', coreLabel: 'Kernvorteile', legalTitle: 'Risiko- und technische Einordnung', legalIntro: 'Lesen Sie vor dem Fortfahren die folgende Erklärung.', legalBody: 'AI-Quant Lab konzentriert sich auf MQL5-Quellcodeentwicklung und Bildung zu AI-modularem Prompt Engineering. Inhalte dienen Forschung und technischem Austausch. Wir bieten weder Copy Trading noch Vermögensverwaltung oder Gewinnzusagen und verwalten keine Kundengelder. Handelsentscheidungen und Risiken liegen beim Nutzer.', faqTitle: 'Klare Antworten erleichtern den nächsten Schritt', faqBody: 'Häufige Fragen bei Abonnement, MQL5-Lernen und Strategieprüfung. Verstehen Sie zuerst die Grenzen und entscheiden Sie dann sicher.', accordionTitle: 'Risiko- und Abonnementhilfe', accordionBody: 'Zusätzliche Antworten zu Nutzungsanforderungen, Risikogrenzen und Abonnementablauf.', accordionCount: '3 zusätzliche Antworten', faqBadge: 'FAQ / Häufige Fragen', footerPlans: 'Abonnementpläne' } },
  es: { ...homeSecondaryCopy.es, ui: { pricingTitle: 'Resumen de planes de suscripción', pricingBody: 'Cuando la ruta anterior está clara, este es el punto de entrada natural para actuar.', coreLabel: 'Ventajas principales', legalTitle: 'Posicionamiento técnico y de riesgo', legalIntro: 'Lea la siguiente declaración antes de continuar.', legalBody: 'AI-Quant Lab se centra en desarrollo de código fuente MQL5 y educación de ingeniería de prompts AI modular. El contenido se ofrece solo para investigación e intercambio técnico. No ofrecemos copy trading, gestión de operaciones, beneficios garantizados ni manejamos fondos de clientes. Las decisiones y riesgos de trading corresponden al usuario.', faqTitle: 'Las respuestas claras facilitan el siguiente paso', faqBody: 'Preguntas comunes al evaluar una suscripción, aprender MQL5 o revisar una estrategia. Comprenda primero los límites y decida con confianza.', accordionTitle: 'Guía de riesgo y suscripción', accordionBody: 'Respuestas adicionales sobre requisitos de uso, límites de riesgo y flujo de suscripción.', accordionCount: '3 respuestas adicionales', faqBadge: 'FAQ / Preguntas frecuentes', footerPlans: 'Planes de suscripción' } },
});
Object.assign(homeSecondaryCopy, {
  ja: { ...homeSecondaryCopy.ja, ui: { ...homeSecondaryCopy.ja.ui, footerAbout: 'AI-Quant Lab は、MQL5 開発、AI モジュール型プロンプト設計、実践的な定量教育に特化したソースコード定量トレーディングラボです。', footerPill: 'MT5 ソースコード定量の世界を探索', footerModular: 'モジュールブロック', footerPlans: 'サブスクリプションプラン', footerJourneyOne: '内容を理解し、方法を確認してから、次の行動へ進みます。', footerJourneyTwo: 'ここは終着点ではありません。次の定量トレーディングの旅の始まりです。', footerTag: 'AI-Quant Lab ソースコード定量ラボ | MQL5 × AI モジュール型教育' } },
  de: { ...homeSecondaryCopy.de, ui: { ...homeSecondaryCopy.de.ui, footerAbout: 'AI-Quant Lab ist ein Labor für quantitativen Handel mit Quellcode – mit Fokus auf MQL5-Entwicklung, modularer KI-Prompt-Entwicklung und praxisnaher quantitativer Weiterbildung.', footerPill: 'Die quantitative MT5-Quellcode-Welt entdecken', footerModular: 'Modulare Bausteine', footerPlans: 'Abonnementpläne', footerJourneyOne: 'Inhalte verstehen, die Methode erkennen und dann handeln.', footerJourneyTwo: 'Dies ist nicht das Ziel, sondern der Beginn Ihrer nächsten quantitativen Reise.', footerTag: 'AI-Quant Lab Quellcode-Quant-Labor | MQL5 × modulare KI-Bildung' } },
  es: { ...homeSecondaryCopy.es, ui: { ...homeSecondaryCopy.es.ui, footerAbout: 'AI-Quant Lab es un laboratorio de trading cuantitativo con código fuente, centrado en desarrollo MQL5, ingeniería modular de prompts de IA y formación cuantitativa práctica.', footerPill: 'Explora el mundo cuantitativo del código fuente MT5', footerModular: 'Bloques modulares', footerPlans: 'Planes de suscripción', footerJourneyOne: 'Comprende el contenido, conoce el método y después actúa.', footerJourneyTwo: 'Este no es el destino; es el comienzo de tu próximo recorrido cuantitativo.', footerTag: 'Laboratorio cuantitativo de código fuente AI-Quant Lab | MQL5 × educación modular de IA' } },
});
Object.assign(homeSecondaryCopy, {
  ja: { ...homeSecondaryCopy.ja, planHighlights: ['ソースコードを 100% 提供', 'AI LLM プロンプト活用', '1 対 1 の技術ガイダンス'], coreAdvantages: ['アカウント制限、ハードウェア紐付け、コピー取引の著作権制限なしでソースコードを 100% 提供。', 'モジュールブロックとテンプレートによる AI LLM プロンプト活用で、戦略を迅速に再構築。', '公式 LINE アカウントによる 1 対 1 の技術ガイダンス。'] },
  de: { ...homeSecondaryCopy.de, planHighlights: ['100 % Quellcode-Lieferung', 'AI-LLM-Prompt-Aktivierung', '1-zu-1 technische Anleitung'], coreAdvantages: ['100 % Quellcode-Lieferung ohne Kontosperre, Hardwarebindung oder Copy-Trading-Urheberrechtsbeschränkung.', 'AI-LLM-Prompt-Aktivierung mit modularen Bausteinen und Vorlagen für schnellen Strategie-Neuaufbau.', '1-zu-1 technische Anleitung über das offizielle LINE-Konto.'] },
  es: { ...homeSecondaryCopy.es, planHighlights: ['Entrega de código fuente al 100 %', 'Habilitación de prompts AI LLM', 'Guía técnica individual'], coreAdvantages: ['Entrega de código fuente al 100 % sin bloqueo de cuenta, vinculación de hardware ni restricciones de copyright de copy trading.', 'Habilitación de prompts AI LLM con bloques modulares y plantillas para reconstruir estrategias rápidamente.', 'Guía técnica individual mediante la cuenta oficial LINE.'] },
});
Object.assign(homeSecondaryCopy, {
  ja: { ...homeSecondaryCopy.ja, plans: [{ name: '標準メンバーシップ（提供終了）', description: '内容を探索し、定量取引の基礎を築くための実践的な出発点です。', points: ['選定済み MT5 EA 戦略とバックテストレポート', '月次の戦略・ツール選定', 'コミュニティディスカッションへのアクセス'], cta: 'メンバーシップを開く' }, { name: 'プレミアム商用版', description: '戦略をより速く実装し、より実践的なリソースへアクセスしたい利用者向けです。', points: ['完全な MQL5 ソースコード研修とコア戦略ライブラリ', '限定 EA 戦略とテンプレート', '1対1のバックテストおよび展開ガイダンス', 'VIP コア機能の優先アップデート'], cta: 'AI リファクタリングエンジンへ >>>' }], institutional: [{ name: '準機関版（ブローカー、自営取引会社、EA チーム、ファミリーオフィス、私募ファンド向け）', description: '独立した戦略ブランド、チーム取引ワークフロー、完全な製品所有権を構築する準機関運営者向けです。', points: ['ホワイトラベル戦略製品の位置付け支援', '複数口座アーキテクチャと展開例', 'モジュール製品更新への優先アクセス'], cta: 'AI リファクタリングエンジンへ >>>' }, { name: '機関版 — 100% オフライン企業プライベートクラウド', description: 'データ主権、内部ガバナンス、完全にプライベートな定量研究環境を必要とする組織チーム向けです。', points: ['プライベートクラウドとオフライン展開アーキテクチャ', 'アクセス階層、バージョンガバナンス、運用ワークフロー', '長期保守およびアップグレード計画の支援'], cta: 'AI リファクタリングエンジンへ >>>' }] },
  de: { ...homeSecondaryCopy.de, plans: [{ name: 'Standardmitgliedschaft (eingestellt)', description: 'Ein praktischer Einstieg, um Inhalte zu erkunden und ein Fundament im quantitativen Trading aufzubauen.', points: ['Ausgewählte MT5-EA-Strategien und Backtest-Berichte', 'Monatliche Strategie- und Toolauswahl', 'Zugang zu Community-Diskussionen'], cta: 'Mitgliedschaft öffnen' }, { name: 'Premium Commercial', description: 'Für Nutzer, die Strategien schneller umsetzen und auf praxisnähere Ressourcen zugreifen möchten.', points: ['Vollständiges MQL5-Quellcode-Training und Kernstrategiebibliothek', 'Exklusive EA-Strategien und Vorlagen', '1-zu-1 Anleitung für Backtest und Bereitstellung', 'Priorisierte Updates für VIP-Kernfunktionen'], cta: 'Zum AI-Refactoring-Engine >>>' }], institutional: [{ name: 'Quasi-Institutionelle Edition', description: 'Für nahezu institutionelle Betreiber, die eine unabhängige Strategiemarke, Team-Workflow und volle Produkthoheit aufbauen.', points: ['White-Label-Positionierung für Strategieprodukte', 'Multi-Account-Architektur und Bereitstellungsbeispiele', 'Priorisierter Zugang zu modularen Produktupdates'], cta: 'Zum AI-Refactoring-Engine >>>' }, { name: 'Institutionelle Edition — 100 % Offline Enterprise Private Cloud', description: 'Für Teams mit Bedarf an Datenhoheit, interner Governance und vollständig privater quantitativer Forschungsumgebung.', points: ['Private-Cloud- und Offline-Bereitstellungsarchitektur', 'Zugriffsebenen, Versionsgovernance und Betriebsablauf', 'Langfristige Wartungs- und Upgradeplanung'], cta: 'Zum AI-Refactoring-Engine >>>' }] },
  es: { ...homeSecondaryCopy.es, plans: [{ name: 'Membresía estándar (suspendida)', description: 'Un punto de inicio práctico para explorar el contenido y construir una base de trading cuantitativo.', points: ['Estrategias MT5 EA seleccionadas e informes de backtesting', 'Selecciones mensuales de estrategia y herramientas', 'Acceso a discusión de comunidad'], cta: 'Abrir membresía' }, { name: 'Comercial Premium', description: 'Para usuarios que desean implementar estrategias más rápido y acceder a recursos más prácticos.', points: ['Formación completa de código fuente MQL5 y biblioteca de estrategias centrales', 'Estrategias y plantillas EA exclusivas', 'Guía individual de backtesting y despliegue', 'Actualizaciones prioritarias de funciones VIP'], cta: 'Ir al motor de refactorización AI >>>' }], institutional: [{ name: 'Edición cuasi institucional', description: 'Para operadores casi institucionales que construyen una marca de estrategia independiente, flujo de trading en equipo y propiedad completa del producto.', points: ['Guía de posicionamiento de producto estratégico white-label', 'Arquitectura de múltiples cuentas y ejemplos de despliegue', 'Acceso prioritario a actualizaciones modulares'], cta: 'Ir al motor de refactorización AI >>>' }, { name: 'Edición institucional — nube privada empresarial 100 % sin conexión', description: 'Para equipos institucionales que requieren soberanía de datos, gobierno interno y un entorno de investigación cuantitativa totalmente privado.', points: ['Arquitectura de despliegue de nube privada y sin conexión', 'Capas de acceso, gobierno de versiones y flujo operativo', 'Soporte de planificación de mantenimiento y actualización a largo plazo'], cta: 'Ir al motor de refactorización AI >>>' }] },
});
Object.assign(homeSecondaryCopy, {
  ja: { ...homeSecondaryCopy.ja, faqItems: [{ question: 'Q1: 購読後に EA 戦略を使うには特定の FX ブローカーが必要ですか？', answer: 'A1: いいえ。MT5 口座を提供する FX ブローカーであれば利用できます。ソースコードは独立して再利用でき、最適化、コンパイル、再構築が可能です。' }, { question: 'Q2: MQL5 ソースコード研修は完全な初心者にも適していますか？', answer: 'A2: はい。AI LLM と MQL5 構文の基礎から、EA の構築、コンパイル、実践まで段階的に学べます。テンプレートとモジュール例により初心者にも経験者にも取り組みやすい構成です。' }, { question: 'Q3: 標準会員とアフィリエイト会員の戦略はどう違いますか？', answer: 'A3: 標準会員は Clerk のログインと Lemon Squeezy の購読フローを使用し、決済後に Supabase が会員状態とアクセスを自動更新します。アフィリエイト会員の高度な紹介フローは今後拡張予定です。' }, { question: 'Q4: EA 戦略のバックテスト成績とリスク制御は？', answer: 'A4: 公開戦略は五年以上の履歴データ、ウォークフォワード分析、ストレステストを含む検証手順に従います。各戦略には動的 ATR ストップと一取引あたりのリスク上限があり、マーチンゲールやナンピンは採用しません。' }], accordionFaqItems: [{ question: 'Q: AI-Quant Lab を使うには深い Python や MQL5 の経験が必要ですか？', answer: 'A: いいえ。複雑な EA ロジックをモジュールブロックに分解し、専用の AI プロンプトテンプレートと組み合わせています。AI とのガイド付き対話でコア戦略を調整・再構築できます。' }, { question: 'Q: 運用代行、利益保証、投資助言を提供しますか？', answer: 'A: いいえ。AI-Quant Lab はソフトウェア工学と AI 教育のラボです。ソースコードと定量ツールを提供しますが、顧客資金を扱わず、取引判断とリスク管理は利用者自身が行います。' }, { question: 'Q: 購読後はどう始めますか？', answer: 'A: 標準会員はアカウントを作成して購読決済を完了します。Webhook が会員記録を更新するとアクセスが自動有効化されます。まず探索する場合は会員ページからログイン、登録、決済を開始してください。' }] },
  de: { ...homeSecondaryCopy.de, faqItems: [{ question: 'Q1: Benötige ich nach dem Abonnement einen bestimmten Forex-Broker für die EA-Strategien?', answer: 'A1: Nein. Die Strategien funktionieren mit jedem Forex-Broker mit MT5-Konto. Der Quellcode ist unabhängig, wiederverwendbar und kann optimiert, kompiliert oder neu aufgebaut werden.' }, { question: 'Q2: Eignet sich das MQL5-Quellcode-Training für vollständige Anfänger?', answer: 'A2: Ja. Der Lernweg beginnt mit AI-LLMs und MQL5-Syntax und führt über EA-Aufbau und Kompilierung zur praktischen Anwendung. Vorlagen und modulare Beispiele helfen Einsteigern und erfahrenen Nutzern.' }, { question: 'Q3: Wie unterscheiden sich Standard- und Affiliate-Mitgliedschaft?', answer: 'A3: Die Standardmitgliedschaft nutzt Clerk-Anmeldung und Lemon-Squeezy-Abonnement; nach Zahlung aktualisiert Supabase Status und Zugriff automatisch. Der erweiterte Affiliate-Ablauf wird später ergänzt.' }, { question: 'Q4: Wie sind Backtests und Risikokontrollen der EA-Strategien?', answer: 'A4: Veröffentlichte Strategien folgen einem Testverfahren mit mindestens fünf Jahren historischen Daten, Walk-Forward-Analyse und Stresstests. Jede Strategie enthält dynamische ATR-Stopps und ein Risiko-Limit pro Trade, ohne Martingale oder Averaging Down.' }], accordionFaqItems: [{ question: 'Q: Brauche ich tiefe Python- oder MQL5-Erfahrung?', answer: 'A: Nein. Komplexe EA-Logik wird in modulare Bausteine zerlegt und mit AI-Prompt-Vorlagen kombiniert. Geführte AI-Gespräche helfen beim Anpassen und Neuaufbau von Kernstrategien.' }, { question: 'Q: Bieten Sie Vermögensverwaltung, Gewinnzusagen oder Anlageberatung?', answer: 'A: Nein. AI-Quant Lab ist ein Labor für Software Engineering und AI-Bildung. Wir liefern Quellcode und quantitative Werkzeuge, verwalten keine Kundengelder und überlassen Handelsentscheidungen und Risikomanagement dem Nutzer.' }, { question: 'Q: Wie starte ich nach dem Abonnement?', answer: 'A: Standardmitglieder erstellen ein Konto und schließen die Zahlung ab. Nach dem Webhook-Update wird der Zugriff automatisch aktiviert. Zum Erkunden starten Sie auf der Mitgliedschaftsseite mit Anmeldung, Registrierung und Zahlung.' }] },
  es: { ...homeSecondaryCopy.es, faqItems: [{ question: 'Q1: ¿Necesito un bróker forex específico para usar las estrategias EA después de suscribirme?', answer: 'A1: No. Las estrategias funcionan con cualquier bróker forex que proporcione una cuenta MT5. El código fuente es independiente, reutilizable y puede optimizarse, compilarse o reconstruirse.' }, { question: 'Q2: ¿La formación de código fuente MQL5 es adecuada para principiantes?', answer: 'A2: Sí. El recorrido parte de AI LLM y sintaxis MQL5, y avanza por construcción, compilación y uso práctico de EAs. Las plantillas y ejemplos modulares facilitan el acceso a usuarios nuevos y experimentados.' }, { question: 'Q3: ¿En qué se diferencian la membresía estándar y afiliada?', answer: 'A3: La membresía estándar usa inicio de sesión Clerk y suscripción Lemon Squeezy; después del pago, Supabase actualiza automáticamente estado y acceso. El flujo avanzado de afiliados se ampliará más adelante.' }, { question: 'Q4: ¿Cómo son los backtests y controles de riesgo de las estrategias EA?', answer: 'A4: Las estrategias publicadas siguen un proceso con cinco o más años de datos históricos, análisis walk-forward y pruebas de estrés. Cada estrategia incluye stops ATR dinámicos y límite de riesgo por operación, sin martingala ni promediado a la baja.' }], accordionFaqItems: [{ question: 'Q: ¿Necesito experiencia profunda en Python o MQL5?', answer: 'A: No. La lógica EA compleja se divide en bloques modulares y se combina con plantillas de prompts AI. Las conversaciones guiadas con AI permiten ajustar y reconstruir estrategias centrales.' }, { question: 'Q: ¿Ofrecen fondos gestionados, beneficios garantizados o asesoramiento de inversión?', answer: 'A: No. AI-Quant Lab es un laboratorio de ingeniería de software y educación AI. Proporcionamos código fuente y herramientas cuantitativas, nunca manejamos fondos de clientes y las decisiones y riesgos corresponden al usuario.' }, { question: 'Q: ¿Cómo empiezo después de suscribirme?', answer: 'A: Los miembros estándar crean una cuenta y completan el pago. El acceso se activa automáticamente tras la actualización del webhook. Para explorar, comience en la página de membresía con inicio de sesión, registro y pago.' }] },
});
...
export default function Home({ locale = 'en' }) {
  ...
  const copy = localizeChinese(isEnglish ? { ...homeCopy[currentLocale], ...englishDocumentHomepageCopy, ...(secondaryCopy?.ui || {}) } : { ...homeCopy[currentLocale], ...documentHomepageCopy }, currentLocale);
  ...
  const localizedInstitutionalPlans = localizeChinese((isEnglish ? englishInstitutionalPlans : institutionalPlans).map((plan, index) => secondaryCopy?.institutional ? { ...plan, ...secondaryCopy.institutional[index], highlights: secondaryCopy.planHighlights, coreAdvantages: secondaryCopy.coreAdvantages } : (secondaryCopy?.planHighlights ? { ...plan, highlights: secondaryCopy.planHighlights, coreAdvantages: secondaryCopy.coreAdvantages } : plan)), currentLocale);
  const localizedPlans = localizeChinese((isEnglish ? englishPlans : plans).map((plan, index) => secondaryCopy?.plans ? { ...plan, ...secondaryCopy.plans[index], highlights: secondaryCopy.planHighlights } : (secondaryCopy?.planHighlights ? { ...plan, highlights: secondaryCopy.planHighlights } : plan)), currentLocale);
  ...
  const localizedFaqItems = localizeChinese(secondaryCopy?.faqItems || (isEnglish ? englishFaqItems : faqItems), currentLocale);
  const localizedAccordionFaqItems = localizeChinese(secondaryCopy?.accordionFaqItems || (isEnglish ? englishAccordionFaqItems : accordionFaqItems), currentLocale);
  ...
  const ui = secondaryCopy?.ui || (isEnglish ? englishUi : null);
  ...
  return (
    <div className="home-page relative min-h-screen overflow-hidden bg-slate-950 text-slate-100 selection:bg-cyan-500 selection:text-slate-950">
      <div aria-hidden="true" className="home-page-scan-line" />
      ...
      >
        <section className="hero-aurora animate-reveal-up relative z-10 mx-auto flex min-h-[74vh] max-w-5xl flex-col items-center justify-center gap-6 py-12 text-center sm:min-h-[74vh] sm:gap-7 sm:py-16 lg:gap-8">
          ...
          <div className="relative z-10 w-full space-y-7">
            ...
            <div className="mx-auto grid w-full max-w-5xl grid-cols-1 items-stretch gap-4 pt-4 lg:h-[19rem] lg:w-4/5 lg:grid-cols-[2.2fr_1fr] lg:gap-5"><HeroPromoVideo /><div className="grid h-full min-h-0 grid-rows-3 items-stretch gap-3 overflow-visible lg:grid-rows-[repeat(3,minmax(0,1fr))] lg:self-center lg:gap-4">
              <Link href={localizePath('/converter', pageLocale)} className={`hero-cta-glow inline-flex min-h-0 h-full w-full items-center justify-center rounded-xl border border-white/80 bg-gradient-to-r from-[#031f2c] via-[#05343d] to-[#070d1d] px-6 py-3 text-center text-sm font-bold text-cyan-50 shadow-[0_0_9px_rgba(255,255,255,0.36),0_0_34px_rgba(34,211,238,0.22)] transition duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:border-cyan-100 hover:from-[#062b3c] hover:via-[#07505c] hover:to-[#0b1530] hover:shadow-[0_0_4px_rgba(255,255,255,0.16),0_0_48px_rgba(34,211,238,0.48)] ${englishCtaTypography} ${tapClass}`} onClick={() => withTapLock(() => {})}>
                {copy.premiumCta}
              </Link>
