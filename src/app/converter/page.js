...
export default function MqlConverter() {
  ...
  const converterNotices = {
    en: 'Premium Commercial Edition notice: this page is an interactive front-end demonstration. It can display sample MQL5 output and logs, but it is not connected to a live AI model API or MetaEditor compilation service. Visitors may try up to five sample source-code conversions per day with their own valid AI LLM and API credentials.',
    'zh-Hant': '尊爵版說明：本頁為可互動的前端展示，會呈現範例 MQL5 輸出與日誌；目前未連接實際 AI 模型 API 或 MetaEditor 編譯服務。訪客可使用自己的有效 AI LLM 與 API 憑證，每日試用最多五次範例原始碼轉換。',
    'zh-Hans': '尊爵版说明：本页为可交互的前端展示，会呈现示例 MQL5 输出与日志；目前未连接实际 AI 模型 API 或 MetaEditor 编译服务。访客可使用自己的有效 AI LLM 与 API 凭证，每日试用最多五次示例源代码转换。',
    ja: 'プレミアム商用版のお知らせ：このページは対話型フロントエンドのデモです。サンプルの MQL5 出力とログを表示しますが、実際の AI モデル API や MetaEditor のコンパイルサービスには接続していません。有効な AI LLM と API 認証情報を利用して、サンプルソースコード変換を 1 日最大 5 回試せます。',
    de: 'Hinweis zur Premium-Commercial-Edition: Diese Seite ist eine interaktive Frontend-Demonstration. Sie zeigt beispielhafte MQL5-Ausgaben und Protokolle, ist jedoch nicht mit einer echten KI-Modell-API oder der MetaEditor-Kompilierung verbunden. Mit eigenen gültigen AI-LLM- und API-Zugangsdaten sind bis zu fünf Beispiel-Quellcodekonvertierungen pro Tag möglich.',
    es: 'Aviso de la edición comercial prémium: esta página es una demostración interactiva de interfaz. Puede mostrar resultados y registros de ejemplo de MQL5, pero no está conectada a una API real de IA ni a la compilación de MetaEditor. Con credenciales válidas propias de AI LLM y API, se pueden probar hasta cinco conversiones de código fuente de ejemplo al día.',
